...
export interface ExportedDesktopDateRangeCalendarProps<TDate> {
  ...
  renderDay?: (
    day: TDate,
    dateRangePickerDayProps: DateRangePickerDayProps<TDate>
  ) => JSX.Element
}
...
export interface DesktopDateRangeCalendarProps<TDate>
  extends ExportedDesktopDateRangeCalendarProps<TDate>,
    Omit<
      ...
      'selectedDays' | 'renderDay' | 'onFocusedDayChange' | 'classes'
    >,
...
function getCalendarsArray(
  calendars: ExportedDesktopDateRangeCalendarProps<unknown>['calendars']
) {
  switch (calendars) {
    case 1:
      return [0]
 ...
 */
export function DateRangePickerViewDesktop<TDate>(
  inProps: DesktopDateRangeCalendarProps<TDate>
) {
  ...
  const {
    ...
    renderDay = (_, dateRangeProps) => (
      <DateRangePickerDay {...dateRangeProps} />
    ),
  ...
  const leftArrowButtonText =
    leftArrowButtonTextProp ?? localeText.previousMonth
  ...
  const [rangePreviewDay, setRangePreviewDay] = React.useState<TDate | null>(
    null
  )
  ...
  const handleSelectedDayChange = React.useCallback<
    DayPickerProps<TDate>['onSelectedDaysChange']
  >(
    (day) => {
    ...
    <DateRangePickerViewDesktopRoot className={clsx(className, classes.root)}>
      {getCalendarsArray(calendars).map((_, index) => {
        const monthOnIteration = utils.setMonth(
          currentMonth,
          utils.getMonth(currentMonth) + index
        )
            ...
            <DateRangePickerViewDesktopCalendar<TDate>
              ...
              renderDay={(day, __, DayProps) =>
                renderDay(day, {
                  ...
                  isStartOfHighlighting: isStartOfRange(
                    utils,
                    day,
                    parsedValue
                  ),
